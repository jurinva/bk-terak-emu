...
char*
rd_c_addr( char *s, c_addr *v, int *good) 
{
	while( isspace( *s ))
		++s;
	*v = 0;
...
char*
rd_d_word( char *s, d_word *v, int *good)
{
	while( isspace( *s ))
		++s;
	*v = 0;
...
ui_load(char *s)
{
	...
	while (len > 0 && !feof(f)) {
		c1 = getc(f);
		c2 = getc(f);
		if (CPU_OK != sc_word(addr, c2 << 8 | c1)) {
		    break;
		}
		addr += 2;
		len -= 2;
